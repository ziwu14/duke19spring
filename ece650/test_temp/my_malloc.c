...
__thread int empty_heap_nolock = 1;
char * heap_end_nolock = NULL;
...
//////////////////////////no lock version////////////////////////////////////////
void split_block_nolock(meta_t cb,size_t rqt_size){
  ...
  if(first_free_nolock == cb){//corner case
    first_free_nolock = nb;
  }
}
...
meta_t create_block_nolock(size_t rqt_size){
  ...
  printf("nb: %p  heap_end: %p",nb, heap_end);
  heap_end_nolock = (char *)nb + BLOCK_SIZE + rqt_size;
  printf(" nb: %p  heap_end: %p\n",nb, heap_end);
  ...
  if(empty_heap_nolock == 1){
    empty_heap_nolock = 0;
    first_block_nolock = nb;
  }
  printf("bf_create_return\n");
  return nb;
}
...
void * ts_malloc_nolock(size_t size){
  ...
  meta_t cb;
  printf("bf_malloc\n");
  if(first_free_nolock != NULL) {//if the list is not empty, search for a fitting block first
    printf("bf_find_block\n");
    cb = find_block_nolock(rqt_size);//the only difference from ff
    if(cb != NULL) {//if the fitting block is found, we should kick it out of the list
      
      if ((cb->dsize - rqt_size) >= ( BLOCK_SIZE + SPLIT_THR)){//block splitting
	printf("bf_split_block\n");
	split_block_nolock(cb, rqt_size);
      }else{
	printf("bf_no_split_block\n");
	if(cb->fprev)
	  cb->fprev->fnext = cb->fnext;
	if(cb->fnext)
	  cb->fnext->fprev = cb->fprev;
	if(first_free_nolock == cb){//corner case: free only existing block
	  first_free_nolock = NULL;
	}
      }
     
    } else {//if not found, create the block
      printf("bf_create_block1\n");
      cb = create_block_nolock(rqt_size);
      if(cb == NULL){//the creation fails returns NULL
...
///////////////////////////////////////free_nolock////////////////
meta_t get_block_nolock(void *ptr) {
  char *tmp;
  tmp = ptr;//char type offset i.e. size in bytes
  return (ptr = tmp -= BLOCK_SIZE);//start of the block; reverse to creation of a block
}

int valid_addr_nolock(void *ptr) {
  if(first_block_nolock != NULL) {
    if(ptr >= first_block_nolock && (char*) ptr < heap_end_nolock) {//the requested address is located between first block and end of the heap
      return ptr == ((char*)(get_block_nolock(ptr)) + BLOCK_SIZE);
    }
  }
  return 0;
}

void locate_free_nolock(meta_t target){
  meta_t cb = first_free_nolock;
  meta_t last;
  if(cb == NULL){//if there is no free block
    first_free_nolock = target;
    target->fprev = NULL;
    target->fnext = NULL;
  }else{//traverse
    while(cb != NULL){
...
and return the pointer to the current block
 */
meta_t coalesce_nolock(meta_t cb){
  cb->dsize += (BLOCK_SIZE + cb->fnext->dsize);
  cb->fnext = cb->fnext->fnext;
  if(cb->fnext){//set the prev
    cb->fnext->fprev = cb;
...
void ts_free_nolock(void *p){
  printf("free_nolock\n");
  meta_t b;
  if(valid_addr_nolock(p)) {//first check if the address is valid

    //insert the block in the list
    b = get_block_nolock(p);//if yes, get the start of the meta block
    locate_free_nolock(b);
    //coalesce prev block
    if(b->fprev && ((char *)b->fprev + BLOCK_SIZE + b->fprev->dsize == (char *)b )){
      if(first_free_nolock == b){
	first_free_nolock = b->fprev;
      }
      b = coalesce_nolock(b->fprev);
    }
    //coalesce next block
    if(b->fnext && ((char *)b + BLOCK_SIZE + b->dsize == (char *)b->fnext )){//if next is NULL
      if(first_free_nolock == b->fnext){
	first_free_nolock = b;
      }
      coalesce_nolock(b);//if no, check if coalesce is available
    }
